...
class Product_Custom_Fields_API
{
    ...
    /**
     * Format field value for display in API response.
     *
     * @since    1.0.0
     * @param    array    $field_data    The field data including type, value, and options.
     * @return   string|array            Formatted value for display.
     */
    private function format_field_value_for_display($field_data)
    {
        $value = $field_data['value'];
        ...
        switch ($field_type) {
            case 'checkbox':
                return $value === 'yes' ? esc_html__('Yes', 'osna-wp-tools') : esc_html__('No', 'osna-wp-tools');

            case 'select':
                // If we have options and the value exists in options, return the label
                if (!empty($field_data['options']) && isset($field_data['options'][$value])) {
                    return $field_data['options'][$value];
                }
                return $value;
